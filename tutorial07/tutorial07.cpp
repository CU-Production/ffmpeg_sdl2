...
int stream_component_open(VideoState * videoState, int stream_index)
{
    ...
    if (codecCtx->codec_type == AVMEDIA_TYPE_AUDIO)
    {
        ...
        // open audio device
        videoState->audio_device = SDL_OpenAudioDevice(nullptr, 0, &wanted_specs, &specs, 0);
        SDL_PauseAudioDevice(videoState->audio_device, 0);
        // check audio device was correctly opened
        if (!videoState->audio_device)
        {
...
void video_display(VideoState * videoState)
{
    ...
    if (videoPicture->frame)
    {
        ...
        if (++videoState->currentFrameIndex < videoState->maxFramesToDecode)
        {
            ...
            rect.x = 0;
            rect.y = 0;
            rect.w = videoState->video_ctx->width;
            rect.h = videoState->video_ctx->height;
